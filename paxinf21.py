...
def obter_sequencia_do_arquivo():
    ...
    # Usa uma expressão regular para extrair o número do arquivo (por exemplo, "03" de "pax03.py")
    match = re.match(r"paxinf(\d{2})\.py", nome_arquivo)
    
    if match:
        return match.group(1)  # Retorna o número da sequência (como "03")
    else:
        raise ValueError("Nome do arquivo não segue o formato esperado (paxXX.py).")
...
scripts = [f"{script}{sequencia}.py" for script in scripts_base]
